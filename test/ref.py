...
class ModelArguments:
    model_name_or_path: Optional[str] = field(default="/home/lixz23/pretrain-model/minicpm-2b-sft-bf16")
...
class DataArguments:
    train_data_path: str = field(
        default="/data/groups/QY_LLM_Other/lixinze/icrl_2024/rerank/train_LLM_data/minicpm_top5/marco_LLM_rerank/dpo_top_1-5_train.jsonl",
        metadata={"help": "Path to the training data."},
    )
    eval_data_path: str = field(
        default="/data/groups/QY_LLM_Other/lixinze/icrl_2024/rerank/train_LLM_data/minicpm_top5/marco_LLM_rerank/dpo_top_1-5_dev.jsonl",
        metadata={"help": "Path to the test data."},
    )
    
    max_length: int = field(default=256,metadata={"help":"Maximum all sequence length."},)
...
def preprocessing(example,args,tokenizer):
        ...
        if example['data_type'] not in ['rag_bench']:
            ...
            if len(example['rerank_passage'])>= args.top_n:
                psgs = example['rerank_passage'][:args.top_n]#item['rerank_passage']代表我们使用rerank之后的passage
            else:
