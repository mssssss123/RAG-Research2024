    ...
    def get_more_psg(self,item,raw_input):
        ...
        for idx, psg in enumerate(psg_list):
            if self.args.llama_style:
                ...
                if self.args.chat_templete:
                    aug_input = [{"role": "user", "content": augment_templeta.format(psg, raw_input)}]
...
class MyArgument:    
    ...
    input_path: str= field(
                        default='/data/groups/QY_LLM_Other/meisen/rerank_test/train_reranktop10/dpo_train.jsonl')   

    out_path: str= field(default="/data/groups/QY_LLM_Other/meisen/rerank_test/train_reranktop10/temp")
    batch_size: int = field(default=1)
    loop: int = field(default=5)
    
    cut_chunk: int = field(default=4)
    number_chunk: int = field(default=1)
    
    llama_style: bool = field(default=False)
    chat_templete: bool = field(default=True)
...
def main():
    ...
    parser.add_argument('--output_path', type=str,
                        default="/data/groups/QY_LLM_Other/meisen/rerank_test/train_reranktop10/temp")
