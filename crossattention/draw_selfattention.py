...
def get_example(data,docs,cap,img_tsv,img_map):
    ...
    for idx, item in enumerate(tqdm(data)):
        ...
        if len(pos_text_list)+len(pos_img_list)>=len(data):
            break
    pos_img_list = pos_img_list[:2500]
...
def get_self_attention(model,tokenizer,image_processor,pos_text_list,pos_img_list,device):
    ...
    # TODO:self_attention entropy
    step_batch = 100

    all_attention = []
    for jj in range(25):
        one_image_batch = img_all[int(step_batch * jj):int(step_batch * (jj + 1)), ...]
        one_cap = cap_all[int(step_batch * jj):int(step_batch * (jj + 1))]
        one_cap_batch = tokenizer(one_cap, return_tensors='pt', max_length=cap_len, padding='max_length',
                                  truncation=True, add_special_tokens=False).to(device)
        with torch.no_grad():
            v = model(one_image_batch, one_cap_batch, device, None,True)
        ...
        print("-----------------------------")
    combined_all_attention = torch.cat(all_attention, dim=0)
    # combined_all_attention = torch.stack(all_attention, dim=0)
    # combined_all_attention = torch.cat(all_attention)
    print("-------------------")
    attention_entropy_all=[]
    # average_tensor = torch.mean(combined_all_attention, dim=0)
    for single_attention in combined_all_attention:

        attentions_tmp = single_attention + 1e-8
        attention_entropy = (- attentions_tmp * torch.log(attentions_tmp)).sum(-1).mean()
        attention_entropy_all.append(attention_entropy.cpu().item())
    avage_attention_entropy=np.mean(attention_entropy_all)
    singel = True
    if singel:
        ...
        for layer in v:
            attentions += layer
        attentions_tmp = attentions.squeeze()
        attentions_tmp = attentions_tmp + 1e-8
        attention_entropy = (- attentions_tmp * torch.log(attentions_tmp)).sum(-1).mean()
        ...
        for idx in range(len(cap_all)):
            ...
            text = cap_all
            v = model(img_one, input_cap_one, device, None, True)
